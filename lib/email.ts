...
export async function sendTransactionalEmail({
  ...
  const fromEmail = integrations?.resend?.fromEmail || process.env.RESEND_FROM_EMAIL || "noreply@groundlypro.com";

  if (!apiKey) throw new Error("No email provider configured. Set up Gmail or Resend in Settings → Integrations.");
