...
import { Edit, Mail, MessageSquare, MoreHorizontal, Phone, Trash2 } from "lucide-react";
...
export function ClientRowActions({
  ...
  async function handleSms() {
    ...
    try {
      ...
      window.alert(error instanceof Error ? error.message : "Unable to open Quo contact.");
    } finally {
      setOpeningQuo(null);
    }
  }
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-44">
          ...
          <DropdownMenuItem onClick={() => void handleCall()} disabled={!phone || openingQuo !== null}>
            <Phone className="size-4" />
            {phone ? (openingQuo === "call" ? "Opening Quo..." : "Call") : "No phone"}
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => void handleSms()} disabled={!phone || openingQuo !== null}>
            <MessageSquare className="size-4" />
            {phone ? (openingQuo === "sms" ? "Opening Quo..." : "SMS") : "No phone"}
          </DropdownMenuItem>
          <DropdownMenuSeparator />
